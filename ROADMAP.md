...
| M7 | Pass the OCaml testsuite — full bytecode suite under StickyImmix: **1476/1524 pass** (`setarch -R`, per-dir 120s cap). 47 non-pass are unsupported features (weak/finaliser → fixed by `MMTK_WEAK_REFS`; `Gc.stat`/memprof/runtime-events) — none crash; 1 real regression = `parallel/domain_parallel_spawn_burn_gc_set` SIGSEGV under StickyImmix only (bug #3, multidomain+moving). Default Immix passes the suite, **but the latent moving-GC bug (#2) still bites rarely on Immix** — it is what SEGVs the linux-arm64 CI `Build` at the ocamldoc man-page step (StickyImmix triggers it ~25%). Heavily narrowed (see `gc/mmtk/NOTES.md`): it is an **Immix in-place partial-defrag** bug (MarkSweep / GenImmix / Immix-`ALWAYS_DEFRAG` all clean — only *opportunistic* in-place moving crashes). Symptom pinned (rr + a `MMTK_DEBUG_OFFBY8` heap detector): a reference one header-word **too low** — an object *start* used where a `value` (start+8) belongs → bad `SWITCH` tag → control-flow desync. Ruled out (audited correct): roots, object copy, classify/load/store, the write barrier (incl. region-barrier over-conservative-but-safe under StickyImmix), stack relocation, `Alloc_small` Setup/Restore, **weak/ephemeron/finaliser forwarding** (`MMTK_WEAK_REFS=0` A/B still crashes), and **`FieldSlot::store`** (detector: 0 store-side hits). Separately, a **real latent `Is_young`-gated barrier elision was found+fixed in `caml_uniform_array_make` (`runtime/array.c`)** — `Is_young` is always false under MMTk, so it was skipping the generational barrier — but that is NOT this crash (it persists with the fix). Next: rr on `copy_object` during a partial-defrag GC. See `gc/mmtk/NOTES.md`. | 🟡 |
