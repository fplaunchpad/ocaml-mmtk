...
| `MMTK_MEDIUM_TO` | `immix` | EXPERIMENTAL: `freelist` routes the pretenured ≥2056B band to the common mark-sweep space — vanilla's reclamation regime (fragmed's root fix) — but is UNSOUND under concurrent cycles as of round 30d (lazy sweep vs in-flight marks; see NOTES). Keep the default. |
