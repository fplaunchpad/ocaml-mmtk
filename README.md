...
> **See also `quick/`** — a small, fast **quick-decision GC panel** (eleven benches +
> a one-command `quickbench.py` harness that runs, tables, and graphs in one shot)
> for getting rapid perf signal while iterating on a GC change (e.g. the no-zero
> allocation change). It complements, does not replace, this CLBG correctness suite
> and the heavyweight ocaml-bench/macro-bench authoritative suite. Details:
> [`quick/README.md`](quick/README.md).
