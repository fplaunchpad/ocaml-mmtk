...
| M6 | Runtime features: weak arrays, ephemerons, finalisers — `process_weak_refs` gated by `MMTK_WEAK_REFS` (default off = memory-safe interim). Flag-on: weak-clear, ephemeron-release, `Gc.finalise`/`finalise_last`, **and custom-block finalizers** (`Custom_operations.finalize`, incl. unmarshalled) all work — `pr3612` passes. Remaining: one weak+finaliser resurrection case (`pr5233`, over-retention) + finaliser-timing edge cases; full re-measurement in progress | 🟡 mostly working (gated) |
