...
| M6 | Runtime features: weak arrays, ephemerons, finalisers — `process_weak_refs` **partial**, gated by `MMTK_WEAK_REFS` (default off = memory-safe interim). Flag-on: basic weak-clear / ephemeron-release / `Gc.finalise` work (smoke), but full suite fixes only 1/8 targeted tests — **custom-block finalizers unimplemented** + weak+finaliser ordering off. Not ready to gate the major-GC removal | 🟡 partial (gated) |
