...
| M6 | Runtime features: weak arrays, ephemerons, finalisers — `process_weak_refs` gated by `MMTK_WEAK_REFS` (default off = memory-safe interim). Flag-on: weak-clear, ephemeron-release, `Gc.finalise`/`finalise_last`, **and custom-block finalizers** all work under Immix **and** StickyImmix — `pr3612` + `pr5233` pass; no flag-attributable regressions. (Remaining testsuite failures are non-M6: memprof, runtime-events, `Gc.stat`.) Default-flip + stage-3 removal next | 🟢 working (gated) |
