...
let test_weak () =
  ...
  Weak.set w 0 (Some (Bytes.create 1000));   (* only the weak slot references it *)
  churn (); churn ();
  match Weak.get w 0 with
  | None   -> Printf.printf "weak: CLEARED (correct weak semantics)\n%!"
  | Some _ -> Printf.printf "weak: still alive (weak processing not wired => strong)\n%!"
...
let test_ephemeron () =
  ...
  | Some _ -> Printf.printf "ephemeron: data retained while key live (ok, no crash)\n%!"
  | None   -> Printf.printf "ephemeron: data cleared while key live (WRONG)\n%!"

let test_finalise () =
  let fired = ref 0 in
  let mk () = let b = Bytes.create 1000 in Gc.finalise (fun _ -> incr fired) b; b in
...
let test_gc_module () =
  (try Gc.full_major (); Printf.printf "Gc.full_major: ok\n%!"
   with e -> Printf.printf "Gc.full_major: raised %s\n%!" (Printexc.to_string e));
  (try let s = Gc.stat () in Printf.printf "Gc.stat: live_words=%d\n%!" s.Gc.live_words
