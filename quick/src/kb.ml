(* knuth-bendix completion of the geometry-group axioms (OCaml testsuite
   testsuite/tests/misc-kb/, inlined: terms+equations+orderings+kb+main).
   A term-rewriting / symbolic workload (Rocq/Coq-like allocation profile:
   many small short-lived terms + a growing rule set). Sequential.
   The original prints the canonical rule set; we redirect that output into a
   rolling checksum so the result is a single stable line. arg1 = iterations. *)
let kb_cksum = ref 0
let kb_acc s = String.iter (fun c -> kb_cksum := (!kb_cksum * 33 + Char.code c) land 0x3FFFFFFF) s
let print_string s = kb_acc s
let print_int n = kb_acc (string_of_int n)
let print_newline () = kb_acc "\n"
(****************** Term manipulations *****************)
