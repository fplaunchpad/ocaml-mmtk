 ...
 *
 * Verbatim port of sandmark benchmarksgame/fannkuchredux.ml. argv[1] = n.
 *)
...
module Perm =
  ...
  let next { p = p; c = c; _ } =
    ...
    while !aug_c > !i do
      ...
      (* Upstream sandmark indexes c.(!i)/p.(!i) one past the array here when the
         carry runs off the end of a length-[plen] permutation, crashing with
         Invalid_argument at every n. Guard the carry index the same way the
         p.(!i) write above already is: at the boundary the chunk's enumeration
         is exhausted, so stop. *)
      if plen > !i then begin
        aug_c := c.(!i) + 1;
        c.(!i) <- !aug_c
      end else
